...
public class UserServiceImpl implements UserService {
    ...
    public UserDto createUser(UserDto userDto) {
        if (Objects.nonNull(userDto)) {
            if (userRepository.findByEmailAndEnabled(userDto.getEmail(),1).isPresent())
    ...
    public List<UserDto> getAllUsers() {
        return userMapper.mapToUserDtos(userRepository.findAllByEnabledOrderByCreatedDateAsc(1));
    }
    ...
    public UserDetailsService userDetailsService() {
        return username -> (UserDetails) userRepository.findByEmailAndEnabled(username,1)
